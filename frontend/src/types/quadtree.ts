...
export class BoundingBox {
	...
	public containsPoint = (point: THREE.Vector3): boolean => {
		return (
			point.x >= this.x - this.w && point.x < this.x + this.w && point.y >= this.y - this.h && point.y < this.y + this.h && point.z >= this.z - this.l && point.z < this.z + this.l
		);
	};
...
export class QuadTree {
	...
	private subdivide = (): void => {
		...
		const NW: BoundingBox = new BoundingBox(x - halfW, y + halfH, z - halfL, halfW, halfH, halfL);
		const NE: BoundingBox = new BoundingBox(x + halfW, y + halfH, z - halfL, halfW, halfH, halfL);
		const SW: BoundingBox = new BoundingBox(x - halfW, y + halfH, z + halfL, halfW, halfH, halfL);
		const SE: BoundingBox = new BoundingBox(x + halfW, y + halfH, z + halfL, halfW, halfH, halfL);
	...
	public drawQuadTree = (scene: THREE.Scene): void => {
		const NW = new THREE.Vector3(this.boundingBox.x - this.boundingBox.w * 2, 0, this.boundingBox.y + this.boundingBox.h * 2);
		const NE = new THREE.Vector3(this.boundingBox.x + this.boundingBox.w * 2, 0, this.boundingBox.y + this.boundingBox.h * 2);
		const SW = new THREE.Vector3(this.boundingBox.x - this.boundingBox.w * 2, 0, this.boundingBox.y - this.boundingBox.h * 2);
		const SE = new THREE.Vector3(this.boundingBox.x + this.boundingBox.w * 2, 0, this.boundingBox.y - this.boundingBox.h * 2);
